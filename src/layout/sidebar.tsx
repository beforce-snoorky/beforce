...
export async function Sidebar({ pathname, isAdmin }: { pathname: string, isAdmin: boolean }) {
  return (
    <aside className="max-xl:hidden flex flex-col z-50 p-4 max-w-56 border-r border-surface bg-light">
      <NavList title="Geral" items={userNavigation} pathname={pathname} />
      {isAdmin && (
        <>
          <hr className="mt-4 border-t border-surface" />
          <NavList title="Administração" items={adminNavigation} pathname={pathname} />
        </>
      )}
